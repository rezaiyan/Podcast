...
public class PersianCalendar extends GregorianCalendar {
    ...
    public void addPersianDate(int field, int amount) {
        ...
        if (field == YEAR) {
            ...
            setPersianDate(this.persianYear + ((getPersianMonth() + amount) / 12), (getPersianMonth() + amount) % 12,
                    this.persianDay);
            return;
        }
        add(field, amount);
    ...
    public String getPersianLongDate() {
        return getPersianWeekDayName() + "  " + this.persianDay + "  " + getPersianMonthName() + "  "
                + this.persianYear;
    }
    ...
    public String getPersianShortDateTime() {
        return "" + formatToMilitary(this.persianYear) + delimiter + formatToMilitary(getPersianMonth()) + delimiter
                + formatToMilitary(this.persianDay) + " " + formatToMilitary(this.get(HOUR_OF_DAY)) + ":"
    ...
    public PersianCalendar setPersianDate(int persianYear, int persianMonth, int persianDay) {
        ...
        setTimeInMillis(convertToMilis(PersianCalendarUtils
                .persianToJulian(this.persianYear > 0 ? this.persianYear : this.persianYear + 1,
    ...
    protected void calculatePersianDate() {
        ...
        long julianDate = (
                (long) Math.floor((getTimeInMillis() + ZONE_OFFSET - PersianCalendarConstants.MILLIS_JULIAN_EPOCH))
    ...
    private long convertToMilis(long julianDate) {
        return PersianCalendarConstants.MILLIS_JULIAN_EPOCH
                ...
                + PersianCalendarUtils.ceil(getTimeInMillis() - PersianCalendarConstants.MILLIS_JULIAN_EPOCH,
                PersianCalendarConstants.MILLIS_OF_A_DAY);
    }
